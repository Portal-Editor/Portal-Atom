...
export default class PortalClient {
  constructor(props) {
    this.props = props;
    const {portalId, userId, projectPath, projectDir, emitter, portal, initNewPortal} = props;
  ...
  init () {
    ...
    this.portal.subscriptions.add(atom.project.onDidChangeFiles(events => {
      for (const i in events) {
        const event = events[i]
        let path = this.getNomalizedRelativePath(event.path)        
        if (event.action === 'renamed') {
          let oldPath = this.getNomalizedRelativePath(event.oldPath)
          this.sendSocketMsg(renameFileAction(path, oldPath))
          console.log(`Atom Event: renamed from ${event.oldPath} to ${event.path}`)
        }
        else {
          console.log(`Atom Event: ${event.action} ${path}`)
        }
      }
    }))
  }
