...
export default class PortalClient {
    ...
    switch (data.type) {
      case ACTION_TYPES.INIT:
        ...
        if (this.initNewPortal && data.portalId) {
          ...
          try {
            const unzipStream = unzipper.Extract({path: this.projectPath})
  ...
  disconnect() {
    try {
      this.fileSubscriptions && Object.values(this.fileSubscriptions).forEach(subscription => subscription && subscription.dispose())
      this.eventHandlers && Object.values(this.eventHandlers).forEach(handler => handler && handler.destroy())
      this.users && Object.values(this.users).forEach(user => user.badge && user.badge.element.remove())
      this.files && Object.values(this.files).forEach(file => file.lockView && file.lockView.remove())
      this.currentUserBadge && this.currentUserBadge.element.remove()
      this.globalSubscriptions && this.globalSubscriptions.dispose()
